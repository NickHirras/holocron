...
- [ ] **Question Endpoints**: Ensure `CeremonyQuestionResource` supports:
    ...
    - `POST /ceremonies/{id}/questions/reorder`: Accept a list of question IDs in their new order and update `sequence`.
...
- [ ] Integration of `SortableJS` (via CDN or local asset).

## UX Considerations
- **"Google Forms" Feel**: Clean cards, distinct shadows on hover, smooth transitions.
- **Feedback**: "Saved" indicators when editing fields.
